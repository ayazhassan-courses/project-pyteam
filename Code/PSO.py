...
def PSO():
    ...
    Num_Iterations= 50      # number of particles in search space
    Num_Particles= 25       # number of iterations to be performed
    Num_dimensions = 5      # variables in function
    w= 0                    # constant weight inertia to vary the velocity in each iteration
    wMin,wMax= 0.5,0.9      # lower and upper bound of weight inertia to increase or decrease velocity in search space
    ...
    pbest_val_i = [1000000 for i in range (Num_Particles)]         #setting personal best value for each particle
    pbest_pos_i = [[0 for i in range (Num_dimensions)] for i in range (Num_Particles)] #personal best position of each
                                                                                   # particle in each dimension
    gbest_pos = [0 for i in range (Num_dimensions)]                       # global best position of the swarm

    X = initial_position(Num_Particles, Num_dimensions, xMin, xMax)         #current/initial position of each particle
    V = initial_velocity(Num_Particles, Num_dimensions, vMin, vMax)         #current/initial velocity of each particle
    fitness_i= [sphere(X[i]) for i in range (Num_Particles)]            #fitness of each particle at initial position

    i=0
    while i< Num_Iterations:
        X = update_position(X,Num_Particles,Num_dimensions,xMin,xMax,V)
        fitness_i,gbest_val,gbest_pos,pbest_pos_i,pbest_val_i = Calculate_fitness(X,fitness_i,Num_Particles, pbest_pos_i, pbest_val_i, gbest_pos, gbest_val)
        w = wMax - ((wMax-wMin)/Num_Iterations)*i
        V = update_velocity(X,V,Num_Particles ,Num_dimensions, i, w, vMin, vMax, pbest_pos_i, gbest_pos, c1, c2)
